(function ($) {
  Drupal.stanford_media_crop = Drupal.stanford_media_crop || {};
  Drupal.stanford_media_crop.actions = Drupal.stanford_media_crop.actions || {};

  Drupal.behaviors.stanford_media_crop = {
    attach: function (context) {

      // Process special elements
      Drupal.stanford_media_crop.processSpecialSelect(context);
      ...
      var imageStyleSafe = Boolean(Drupal.settings.stanford_media_crop.styles[($('select[name=image_style]', context).val())] && Drupal.settings.stanford_media_crop.styles[($('select[name=image_style]', context).val())].safe);
      var imageStyleHasCrop = (imageStyleSafe && (Drupal.settings.stanford_media_crop.styles[($('select[name=image_style]', context).val())].crop || false));
      ...
      var getImageScale = Drupal.stanford_media_crop.actions.getImageScale;
      var recalculateScale = Drupal.stanford_media_crop.actions.recalculateScale;
      var setOrigWidth = Drupal.stanford_media_crop.actions.setOrigWidth;
      var setOrigHeight = Drupal.stanford_media_crop.actions.setOrigHeight;
      var setCropWidth = Drupal.stanford_media_crop.actions.setCropWidth;
      var setCropHeight = Drupal.stanford_media_crop.actions.setCropHeight;
      var setScaleWidth = Drupal.stanford_media_crop.actions.setScaleWidth;
      var setScaleHeight = Drupal.stanford_media_crop.actions.setScaleHeight;
      var resetScale = Drupal.stanford_media_crop.actions.resetScale;
      ...
      Drupal.stanford_media_crop.actions.updateRotation = updateRotation;
      ...
      $('select[name=image_style]', context).change(function () {
        ...
        try {
          if (Drupal.settings.stanford_media_crop.styles[selected].safe) {
            imageStyleSafe = true;
            toggleImageStyleWarning();
            activeCroppingInstance.update();
            var props = Drupal.settings.stanford_media_crop.styles[selected];
      ...
      $('input[name=scale_width]', context).change(function (ev) {
        ...
        if (scaleWidth) {
          ...
          var cropWidth = Drupal.stanford_media_crop.cropWidth || origWidth;
          var cropHeight = Drupal.stanford_media_crop.cropHeight || origHeight;
          var aspectRatio = cropWidth / cropHeight;
          var scaleHeight = scaleWidth / aspectRatio;
          scaleHeight = Math.round(scaleHeight);
          Drupal.stanford_media_crop.actions.setScaleHeightUI(scaleHeight);
      ...
      $('input[name=scale_height]', context).change(function (ev) {
        ...
        if (scaleHeight) {
          ...
          Drupal.stanford_media_crop.actions.setScaleWidthUI(scaleWidth);
          toggleScaleWarning(scaleWidth, scaleHeight);
        }
        else {
          resetScale();
      ...
      var toggleScaleWarning = Drupal.stanford_media_crop.actions.toggleScaleWarning;

      // Store original Drupal.media.formatForm.getOptions() for extending.
      var getOptions = Drupal.media.formatForm.getOptions;

      // Extend Drupal.media.formatForm.getOptions() to incorporate stanford_media_crop
      // related settings.
      Drupal.media.formatForm.getOptions = function () {
        var options = getOptions();

        var mediaCropSettings = {
          stanford_media_crop_rotate: Number(($('input[name=rotate]', context).val() || '0')),
          stanford_media_crop_x: Number(($('input[name=x]', context).val() || '0')),
          stanford_media_crop_y: Number(($('input[name=y]', context).val() || '0')),
          stanford_media_crop_w: Number(($('input[name=width]', context).val() || '0')),
          stanford_media_crop_h: Number(($('input[name=height]', context).val() || '0')),
          stanford_media_crop_scale_w: Number(($('input[name=scale_width]', context).val() || '0')),
          stanford_media_crop_scale_h: Number(($('input[name=scale_height]', context).val() || '0')),
          stanford_media_crop_image_style: getImageStyle(),
          stanford_media_crop_instance: '%7BMCIID%7D'
        };
      ...
      Drupal.media.formatForm.getFormattedMedia = function () {
        var formatType = ($('input[name=format]', context).val() || 'stanford_media_crop');
        ...
        var id = 'stanford_media_crop_' + String(r - (r % 1));
        ...
        parent.window.Drupal.stanford_media_crop.replaceImage(replacedData);

        var mediadata = {
          type: formatType,
          options: options,
          html: Drupal.settings.stanford_media_crop.imageStyleHtml[options.stanford_media_crop_image_style]
        };
  ...
  Drupal.stanford_media_crop.actions = {
    setOrigWidth: function (width) {
      width = Number(width);
      Drupal.stanford_media_crop.origWidth = width;
    },
    setOrigHeight: function (height) {
      height = Number(height);
      Drupal.stanford_media_crop.origHeight = height;
    },
    setCropWidth: function (width) {
      width = Number(width);
      Drupal.stanford_media_crop.cropWidth = width;
      $('input[name=width]').val(width);
    },
    setCropHeight: function (height) {
      height = Number(height);
      Drupal.stanford_media_crop.cropHeight = height;
      $('input[name=height]').val(height);
    },
    setScaleWidth: function (width) {
      width = Number(width);
      Drupal.stanford_media_crop.scaleWidth = width;
    },
    setScaleHeight: function (height) {
      height = Number(height);
      Drupal.stanford_media_crop.scaleHeight = height;
    },
    ...
    recalculateScale: function (selection) {
      if (selection.width !== Drupal.stanford_media_crop.cropWidth || selection.height !== Drupal.stanford_media_crop.cropHeight) {
        Drupal.stanford_media_crop.actions.resetScale();
      }
    },
    toggleScaleWarning: function (scaleWidth, scaleHeight) {
      var imageScale = Drupal.stanford_media_crop.actions.getImageScale();
      var cropWidth = Drupal.stanford_media_crop.cropWidth ? Math.round(Drupal.stanford_media_crop.cropWidth / imageScale) : Drupal.stanford_media_crop.origWidth;
      var cropHeight = Drupal.stanford_media_crop.cropHeight ? Math.round(Drupal.stanford_media_crop.cropHeight / imageScale) : Drupal.stanford_media_crop.origHeight;
    ...
    getImageScale: function () {
      return 350 / Math.max(Drupal.stanford_media_crop.origWidth, Drupal.stanford_media_crop.origHeight);
    },
    ...
    init: function (data) {
      var crop = data.crop;
      var isCropRotationScaleEnabled = Boolean(Number(crop.stanford_media_crop_w) || Number(crop.stanford_media_crop_h) || Number(crop.stanford_media_crop_rotate) || Number(crop.stanford_media_crop_scale_w) || Number(crop.stanford_media_crop_scale_h));

      var setCropRotationScale = function () {
        if (isCropRotationScaleEnabled) {
          Drupal.stanford_media_crop.actions.enableInterface();
        }

        if (Number(crop.stanford_media_crop_w) && Number(crop.stanford_media_crop_h) && Number(crop.stanford_media_crop_rotate)) {
          Drupal.stanford_media_crop.actions.updateRotation(Number(crop.stanford_media_crop_rotate), function () {
            $('input[name=x]').val(crop.stanford_media_crop_x);
            $('input[name=y]').val(crop.stanford_media_crop_y);
            Drupal.stanford_media_crop.actions.setCropWidth(crop.stanford_media_crop_w);
            Drupal.stanford_media_crop.actions.setCropHeight(crop.stanford_media_crop_h);

            var selectionData = {
              x1: Number(crop.stanford_media_crop_x),
              x2: Number(crop.stanford_media_crop_x) + Number(crop.stanford_media_crop_w),
              y1: Number(crop.stanford_media_crop_y),
              y2: Number(crop.stanford_media_crop_y) + Number(crop.stanford_media_crop_h)
            };

            Drupal.stanford_media_crop.actions.updateRotation(0, null);
          });
        }
        else if (Number(crop.stanford_media_crop_w) && Number(crop.stanford_media_crop_h)) {
          $('input[name=x]').val(crop.stanford_media_crop_x);
          $('input[name=y]').val(crop.stanford_media_crop_y);
          Drupal.stanford_media_crop.actions.setCropWidth(crop.stanford_media_crop_w);
          Drupal.stanford_media_crop.actions.setCropHeight(crop.stanford_media_crop_h);
        ...
        else if (Number(crop.stanford_media_crop_rotate)) {
          Drupal.stanford_media_crop.actions.updateRotation(Number(crop.stanford_media_crop_rotate), null);
        }

        if (Number(crop.stanford_media_crop_scale_w) || Number(crop.stanford_media_crop_scale_h)) {
          Drupal.stanford_media_crop.actions.setScaleWidthUI(crop.stanford_media_crop_scale_w);
          Drupal.stanford_media_crop.actions.setScaleHeightUI(crop.stanford_media_crop_scale_h);
          Drupal.stanford_media_crop.actions.toggleScaleWarning(crop.stanford_media_crop_scale_w, crop.stanford_media_crop_scale_h);
        }
      };

      if (Drupal.settings.stanford_media_crop.styles[crop.stanford_media_crop_image_style]) {
        $('select[name=image_style]').val(crop.stanford_media_crop_image_style);
        $('select[name=image_style]').change();
        $('a', $('select[name=image_style]').parent()).click();

        if (Drupal.settings.stanford_media_crop.styles[crop.stanford_media_crop_image_style].safe) {
          Drupal.stanford_media_crop.actions.updateRotation(360 - Number($('input[name=rotate]').val()), setCropRotationScale());
        }
        else {
  ...
  Drupal.stanford_media_crop.processSpecialSelect = function (context) {
